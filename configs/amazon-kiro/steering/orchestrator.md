...
- When planning delegated work, name the exact generated role-level agent id, such as `backend-engineer` for implementation and `qa-engineer` for validation.
...
7. You MUST delegate each workflow stage to exact generated role-level agents, such as `backend-engineer` for implementation and `qa-engineer` for validation.
