...
## Skill Routing
- You must read `~/.ai-registry/skills/skills.md`, then `~/.ai-registry/skills/catalog/roles/release-manager/skills.md`, and must use matching skills that directly apply to the assigned role, task, stack, artifact, or domain. If no matching skill exists, state that no matching skill applies and continue with the role instructions.
