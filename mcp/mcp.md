...
Read `../capability-routing.md` first. This file defines MCP selection, transport, and safety rules after a role, task, catalog path, or unassigned route is known.

## MCP Sources

MCP servers may be agent-native or registry-indexed. Treat both as one capability pool. Prefer an already-connected trusted native server when equivalent; otherwise use this registry's catalogs and referenced server instructions. Do not assume both sources mirror each other.

## Scope Resolution

If a prompt, role file, or parent handoff provides a registry role, task, or explicit MCP catalog path, use only that assigned capability scope. Do not reselect workflow, role, or task. Read the assigned role server catalog first; read the task catalog only when a task id or task catalog path is explicit.

If no registry role, task, or catalog path is assigned, read `mcp/routing.md` only to select relevant MCP servers. This does not change assigned role, workflow, parent instructions, expected outputs, or handoff scope.
...
2. Read the narrowest assigned role or task server index before keyword catalogs.
...
- Use an MCP server only for a directly related technology, service, SaaS product, or data domain.
- Prefer read-only operations. Ask before writes unless the user explicitly requested them.
